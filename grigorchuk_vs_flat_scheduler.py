...
class FairPolicyAgent(torch.nn.Module):
    """Identical neural structure used for both experimental groups."""
    def __init__(self, dim=32):
        super().__init__()
        self.fc1 = torch.nn.Linear(dim, 128)
        self.fc2 = torch.nn.Linear(128, 64)
        self.fc3 = torch.nn.Linear(64, 4)  # Outputs 4 choices for BOTH systems
...
# ==========================================
# SMT VERIFIER LAYER (POST-HOC AUDIT)
# ==========================================
def verify_trace_via_smt(action_history):
    """Uses Z3 to verify if the discovered trace breaks physical rules."""
    s = Solver()
    horizon = len(action_history)
    
    # Check if we have an empty trace
    if horizon == 0:
        return "Empty Execution History"
        
    # Variables representing the state of each customer across time ticks
    ...
    # Inject environmental laws into SMT matrix
    for t in range(horizon):
        ...
        for c in range(5):
            if c == cust:
                # Rule rules: can only advance step-by-step
                if act == 0:
...
# ==========================================
def execute_controlled_experiment(iterations=200, horizon=40):
    print("====================================================================")
    print("🔬 RUNNING CONTROLLED EXPERIMENT: GRIGORCHUK VS FLAT STRUCTURES")
    print("====================================================================\n")
    
    compiler = TrueBottomUpGrigorchukCompiler(max_depth=5)
    env = RestaurantEnvironmentAgent()
    
    # Define Flat linear operator alternatives (Action 0=+1, Action 1=-1, Action 2=+5, Action 3=0)
    flat_ops = []
    for i in range(4):
    ...
    for mode, operators in [("Grigorchuk Fractal Group", grigorchuk_ops), ("Flat Linear System", flat_ops)]:
        ...
        for iteration in range(iterations):
            ...
            for step in range(horizon):
                ...
                # Apply the current experimental model's matrix transformation
        ...
        print(f"  -> Complete. Highest Score attained: {max(mode_history)}/5 orders.\n")

    # ==========================================
    # STEP 5: VISUALIZE & AUDIT WITH SMT
    # ==========================================
    print("Evaluating Discovered Solutions with SMT Verification Engine...")
    for mode in results:
        audit_res = verify_trace_via_smt(results[mode][1])
        print(f"  * {mode} Trace Audit: {audit_res}")

    # Export analytical graph
    ...
    plt.title('Fair Benchmark: Structural Group Geometry vs. Linear System')
    plt.xlabel('Training Iterations')
    plt.ylabel('Successfully Completed Orders')
    plt.legend()
    plt.grid(True)
    plt.savefig('fair_comparative_analysis.png')
    print("\n📊 PLOT SUCCESSFULLY GENERATED: Saved as 'fair_comparative_analysis.png'")
